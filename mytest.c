...
#include "src/libarith.h"
#include "src/libec.h"
#include "src/lib_ecc_config.h"
#include "src/lib_ecc_types.h"
#include "src/libsig.h"
//#include "src/words/words.h"
//#include "src/libsig.h"
//#include "src/curves/prj_pt.h"
...
int main(/*int argc, char *argv[]*/)
{
	...
	print_sig_algs();
	printf("\n");
	if(generate_and_export_key_pair("SECP521R1", "ECDSA", "mytestfile")){
			return -1;
	}
	if(sign_bin_file("SECP521R1", "ECDSA", "SHA256", "testfile.txt", "mytestfile_private_key.bin",
			      "signed_testfile.txt", NULL, NULL)){
			return -1;
		}
	if (verify_bin_file("SECP521R1", "ECDSA", "SHA256", "testfile.txt", "mytestfile_public_key.bin", "signed_testfile.txt")) {
			printf("Signature check of %s failed\n", "testfile.txt");
			return -1;
		}

	//Test my scalar mulitiplication
	ret = string_to_params("SECP521R1", "ECDSA", &sig_type, &ec_str_p,
			       NULL, NULL);
	if (ret) {
	...
	import_params(&curve_params, ec_str_p);
	ret = get_file_size("scalar.txt", &buf_len);
	if (ret) {
		printf("Error: cannot retrieve file %s size\n", "scalar.txt");
		return -1;
	}
	ret = nn_get_random_mod(&(kp.priv_key.x), &(curve_params.ec_gen_order));
	if (ret) {
		return -1;
	}
	kp.priv_key.key_type = sig_type;//no used
	kp.priv_key.params = &curve_params;
